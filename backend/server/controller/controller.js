...
exports.createMatch = async (req, res) => {
  await fetch(
    'https://api.football-data.org/v4/competitions/PL/matches?matchday=13',
    {
      headers: {
...
exports.createPrediction = async (req, res) => {
  ...
  if (!req.body) {
    ...
    res
      .status(401)
      .send({ message: 'Unauthorized, you have to login to prediction' });
    return;
  }
  const body = req.body;
  ...
  if (req.query.id) {
    ...
    try {
      ...
      getPredictions.forEach(async (prediction, index) => {
        ...
        if (
          matchDataById.score.fullTime.home >
            matchDataById.score.fullTime.away &&
      ...
      await res.send({
        message: 'Points are updated successfully.',
        matchDataById: matchDataById,
...
exports.deleteMatch = async (req, res) => {
  ...
  if (!id) {
    await res
      .status(404)
      .send({ message: `Error occured id: ${id} doesnt exist` });
    return;
  }
  ...
  try {
    ...
    await res
      .status(404)
      .send({ message: 'Error occured man check again. xd' });
  }
};
